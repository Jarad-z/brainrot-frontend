...
Pairs with a Go backend at `http://localhost:8080` (REST `/api/v1/*` + WebSocket `/ws`). The full engineering baseline is in `docs/FRONTEND.md` and the API contract is in `../backend/docs/API.md` (single source of truth, lives only in backend) — **read these before non-trivial work**.
...
Coverage thresholds (80% lines/branches/functions/statements) apply only to `lib/**`, and `vitest.config.ts` explicitly excludes a handful of files from the denominator: `lib/api/types.ts` (type-only), the thin `auth.ts`/`projects.ts`/`tasks.ts` `apiFetch` wrappers, `lib/ws/provider.tsx` (React Context), `lib/messages.ts` (constants), and `lib/utils.ts` (shadcn `cn()`). Don't fight the exclusion list — exercise these via integration/component tests where they're already covered.
...
- Top-level protected pages: `approvals/` (cross-workspace approval hub), `friends/`, `marketplace/`, `messages/`, `onboarding/`, and the root `page.tsx` (workspace picker that uses `localStorage.lastWsId` as fallback).
- Per-workspace siblings under `w/[wsId]/`: `agents/`, `approvals/`, `runtimes/`, `settings/`, plus the project tree `p/[projectId]/t/[taskId]/`.
...
### Client state: Zustand — three stores, distinct concerns
- `lib/store.ts` — `useAppStore`: current ws/project/task selection + WS connection status only.
- `lib/stores/badges.ts` (plural dir) — unread/pending-approval badge counts.
- `lib/store/chat-ui.ts` (singular dir) — per-task UI state (expanded tool bodies, scroll anchor, active right-rail tab, recent decision records).

Don't conflate the two `store*` directories. None of these hold server data — that all lives in TanStack Query.
...
- Agent create/update (`lib/api/agents.ts`) sends `JSON.stringify(input)` directly — the backend accepts agent jsonb fields (`custom_env`, `custom_args`, `mcp_config`) as raw JSON on POST/PATCH but returns them **base64-encoded** on GET. Decode reads through `lib/codec.ts::decodeJSON<T>`; don't symmetrize the write path to match.
...
`MessageItem.tsx` dispatches by `parsed.type` (from `lib/parse-message.ts`) to subcomponents in `components/chat/parts/`:
- `UserMessage` — user-authored messages (including `@mention` resolution via `MentionedText`).
- `AssistantMessage` — assistant text + thinking blocks.
- `ToolPair` — `tool_use` ↔ `tool_result` paired by `payload.tool_use_id`, rendered as parent/child; show a "running…" skeleton when the result hasn't arrived yet.
- `OrphanToolResult` — `tool_result` with no matching `tool_use` (race / late delivery).
- `PermissionRequestCard` — the visually prominent yellow-border block with a live countdown driven by `expires_at` (server time, never local accumulation).
- `SystemLine`, `ResultBanner`, `RateLimitBanner` — system events, run completion, and rate-limit notices respectively.

Pairing logic lives in `hooks/useToolPairing.ts`; don't re-implement it inside components.
...
Milestone specs live in `docs/superpowers/specs/` (S0 prototype, S1 skeleton, S2 read-only browse, S3 approvals, S4 workspace mgmt, S5 collab+uploads, S6 usability+completeness, plus the `2026-05-21-ui-premium-pixel-redesign.md` visual restraint pass). Backend gaps discovered during frontend work accumulate in `docs/BACKEND_GAPS.md` — append rather than silently work around. When a behavior seems wrong, check BACKEND_GAPS before assuming it's a frontend bug.
