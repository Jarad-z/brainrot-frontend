...
- v1 has no `GET /workspaces` listing and no `GET /workspaces/{wsId}/runtimes` listing (tracked in `docs/BACKEND_GAPS.md`). Workspace selection falls back to `localStorage.lastWsId`; runtime presence is rebuilt purely from WS events (empty until first `runtime.online`).
