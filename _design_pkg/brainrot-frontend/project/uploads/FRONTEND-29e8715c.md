...
- 后端 WS 推 `message.appended` 时，agent 消息会带 `seq`，**按 seq 排序**插入；同一条消息内的多次更新（如果有）以最新 `content` 为准。
- 渲染长 markdown 时**虚拟化列表**（推荐 `@tanstack/react-virtual`）；初始加载用历史拉取 + 分页。
...
| 页面 | 订阅的 scope |
|---|---|
| Workspace 首页 | `workspace:{wsId}` |
| Project 首页 | `workspace:{wsId}` + `project:{projectId}` |
| Task 详情 | `workspace:{wsId}` + `project:{projectId}` + `task:{taskId}` |
...
- 跨域：开发期 Next 端口（`3000`）和 server（`8080`）不同源。后端需要：
  - `Access-Control-Allow-Origin: http://localhost:3000`
  - `Access-Control-Allow-Credentials: true`
  - `SameSite=Lax`（生产同域时不需要 CORS）

  开发期用 Next rewrites 代理 `/api/*` 和 `/ws` 到 `:8080` 避免 CORS（推荐）：
...
6. **同 agent 排队**：`message.metadata.queued = true` 仅是后端的提示；UI 在发送时不要假设，按响应里的 `runs` 字段判断（runs 长度即将启动的 run 数；若为空表示全部被排队）。
...
- [ ] 后端补 `GET /api/v1/workspaces`（当前缺）和 `GET /api/v1/workspaces/{wsId}/runtimes`，前端能正确做首屏选择和 runtime 状态初始化。
